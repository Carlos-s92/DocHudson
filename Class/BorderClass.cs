...
namespace TestGit.Class
{
    internal class BorderClass
    {
        ...
        //--------------------------------------------------------------------------------------------------------------------------------------------------------------------\\
        public void AplicarBordeRedondeadoForm(Control control, float radius, Graphics graph, float borderSize)
        {
            using (GraphicsPath roundPath = GetRoundedPath(new Rectangle(0, 0, control.Width, control.Height), radius))
            using (LinearGradientBrush gradientBrush = new LinearGradientBrush(
                control.ClientRectangle,
                Color.MistyRose,
                Color.Cyan,
                LinearGradientMode.Horizontal))
            using (Pen penBorder = new Pen(gradientBrush, borderSize))
            {
                graph.SmoothingMode = SmoothingMode.AntiAlias;
